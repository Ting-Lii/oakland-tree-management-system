...
-- time complexity:
EXPLAIN ANALYZE SELECT n.name AS neighborhood, COUNT(DISTINCT t.commonName) AS species_count, COUNT(DISTINCT vp.vid) AS total_volunteers
FROM treePlantings tp
    JOIN treeRequests tr ON tp.requestID = tr.requestID
    JOIN neighborhoods n ON tr.neighborhood = n.name
    JOIN treeSpecies t ON TRIM(LEADING ' ' FROM tp.treePlanted) = t.commonName
    LEFT JOIN volunteerPlants vp ON vp.requestID = tp.requestID -- some tree planting might have no volunteer
WHERE YEAR(tp.plantDate) = 2023 AND tr.neighborhood IN (
    SELECT tr2.neighborhood
    FROM treePlantings tp2
        JOIN treeRequests tr2 ON tp2.requestID = tr2.requestID
    WHERE YEAR(tp2.plantDate) = 2023
    GROUP BY tr2.neighborhood
    HAVING COUNT(tp2.requestID) >= 2
)
GROUP BY n.name;
